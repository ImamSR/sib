...
import { connection, findAdminPda, short } from "../../lib/program";
...
export default function ManageAdmins({ program }) {
  const [adminPda] = useMemo(findAdminPda, []);
  ...
  async function load() {
    ...
    try {
      ...
      if (!info) {
        ...
        const admins = (data.admins || []).map((k) => k.toBase58?.() || k);
        setReg({
    ...
    <div className={panelCard}>
      ...
      {loading ? (
        ...
        <div className="mt-5">
          ...
          <button onClick={initRegistry} disabled={busy} className={`${primaryButton} mt-3`}>
        ...
        <div className="mt-5 space-y-5">
          ...
          <div className="rounded-xl border border-white/20 bg-white/50 p-3 backdrop-blur dark:border-white/10 dark:bg-slate-900/30">
            ...
            <div className="mt-3 flex gap-2">
              ...
              />
              <button onClick={addAdmin} disabled={busy || !newAdmin} className={primaryButton}>
                Add
              </button>
            </div>
          </div>
